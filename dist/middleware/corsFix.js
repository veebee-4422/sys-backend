...
exports.corsFix = exports.checkSafeIp = void 0;
const allowedIps = "";
function checkSafeIp(ip) {
    try {
        const thisIpArray = ip.split("."), allowedIpsArray = JSON.parse(allowedIps).map((element) => element.split("."));
        if (thisIpArray.length !== 4)
            return false;
        for (let i = 0; i < allowedIpsArray.length; i++) {
            let flag = false;
            for (let j = 0; j < 4; j++) {
                if (allowedIpsArray[i][j] !== "*" && allowedIpsArray[i][j] !== thisIpArray[j])
                    flag = true;
            }
            if (!flag)
                return true;
...
exports.checkSafeIp = checkSafeIp;
function corsFix(req, res, next) {
